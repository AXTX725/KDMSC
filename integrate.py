...
if __name__ == '__main__':
    ...
    block = [50, 100, 150, 200]     # Select the four optimal scales for ensemble.

    ''' Define the root directory to save ensemble predictions'''
    save_root = 'prediction/{}/{}/integratedLearning_l1_clip'.format(arg.name, arg.spc)   # `save_root` indicates the location where the predicted labels are saved.
    if not os.path.exists(save_root):
        os.makedirs(save_root)
    for r in tqdm(range(arg.run)):
        res = []

        '''Load prediction results from different scales'''
        for b in block:
        ...
        '''Select the class with the highest vote'''
